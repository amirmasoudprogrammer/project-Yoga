...
function CardProduct({data}) {
    return (
        <Link href={`/products/${data.id}`} className="relative hover:shadow-2xl group hover:scale-[0.9] transition-all flex items-start mr-5 justify-start rounded-xl flex-col w-[288px] h-[485px] border border-[#eeeee]">
            <div>
                <Image src={data.featured_image} alt="image" width={288} height={243}
            ...
            <div className="bg-white mt-5 pr-5 pl-5">
                <span className="mt-5 text-black text-[20px] "> {data.name} </span>
                <p className="text-[#8C8C8C] w-[240px] h-[109px] overflow-hidden mb-5 text-[12px] pt-3  font-normal mt-2 border-b border-black whitespace-pre-wrap break-words line-clamp-2">
                    {data.description}
                </p>
            ...
            <div className=' mr-5 mt-4 flex items-center justify-between w-[240px]'>
                ...
                <div className="flex items-center justify-center">
                    <span className="text-[#756CBF] text-[14px] ml-4">{Number(data.price).toLocaleString('fa-IR')}تومان</span>
                </div>
            </div>
        </Link>
    );
