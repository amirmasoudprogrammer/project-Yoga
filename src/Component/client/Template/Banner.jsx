...
function Banner(props) {
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await axios.get(`${BASE_URL}/banners/position/{0}`, {
    ...
    return (
        <>
            <div className="hidden md:block relative z-0 -top-28 h-[800px] ">
                {banners.length === 0 ? (<LoadingOrError message="کمی صبر کنید "/>) : (
                    banners.map((item) => (
                        <Image
                            key={item.id}
                            src={item.image}
  ...
  `}</style>
                ...
                <div
                    className="hidden md:flex bg-white bg-opacity-10 backdrop-blur-md items-center p-3 justify-between mt-2 m-auto absolute right-11 left-3 rounded-lg top-32 text-black w-[250px] h-[100px] border-2">

                    <div
                        className="flex text-[12px] border-l-2 pr-2 pl-2 items-center flex-col justify-center text-white font-bold ">
                        <div>
                    ...
                    <div
                        className="flex text-[12px] border-l-2  pl-2 items-center flex-col justify-center text-white font-bold ">
                        <div>
                ...
                <div
                    className="hidden md:flex border border-white items-center bg-opacity-30 backdrop-blur-[40px] justify-center z-50 bg-[#FFFFFF80] cursor-pointer w-[250px] h-[59px] absolute -bottom-28 left-28 rounded-full">
            ...
            <div className="relative -top-28  z-0  block md:hidden w-full overflow-hidden">
                {/* تصویر اصلی بنر */}
                {banners.length === 0 ? (<LoadingOrError message="کمی صبر کنید "/>) : (banners.map((item) => (<Image key={item.id} src={item.image} alt="بنر تصویر" width={1920} height={1080} quality={100} className="w-full h-[600px] sm:h-[700px] md:h-[950px] object-cover shadow-lg"/>)))}
    ...
    `}</style>
                ...
                {/* اطلاعات آماری */}
                <div
                    className="absolute top-[32%] right-2 z-40 bg-white/10 backdrop-blur-md rounded-lg border w-[37%] max-w-[220px] h-[70px] flex items-center justify-between px-2">
                    {[
                ...
                {/* دکمه مشاهده دوره‌ها */}
                <div
                    className="absolute bottom-48 left-1/2 -translate-x-1/2 z-40 w-[80%] max-w-[250px] h-[50px] rounded-full bg-[#ffffff80] backdrop-blur-xl flex items-center justify-center border border-white shadow-md">
