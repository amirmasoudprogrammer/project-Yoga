...
import PopupTicketSee from "@/Component/admin/modules/PopupTicketSee";
import Cookies from "js-cookie";
import axios from "axios";
import ReplyTicket from "@/Component/admin/modules/ReplyTicket";
...
function TicketPage({data}) {
    ...
    const handleCloseTicket = async (id) => {
        try {
            const token = Cookies.get('tokenAdmin');
            const res = await axios.post(`${BASE_URL}/ticket/tickets/${id}/close`, {
                headers: token ? {Authorization: `Bearer ${token}`} : {}
            });
            console.log(res)
        } catch (error) {
    ...
    const handleReopenTicket = async (id) => {
        try {
            const token = Cookies.get('tokenAdmin');
            const res = await axios.post(`${BASE_URL}/ticket/tickets/${id}/reopen`, {
                headers: token ? {Authorization: `Bearer ${token}`} : {}
            });
            console.log(res)
        } catch (error) {
