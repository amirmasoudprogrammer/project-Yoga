...
function AdminsRoles({puprolesAdmin,setPopupRolesAdmin}) {
    const closePopup = () => setPopupRolesAdmin({ show: false, id: null });
    const [role , setRole] =useState("user")
    const startRoles = async () => {
        try {
            const res = await axios.post(
                `${BASE_URL}/v1/users/${puprolesAdmin.id}/roles`,
                { roles: [role] },
                {
                    headers: {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col gap-6 animate-fade-in">
