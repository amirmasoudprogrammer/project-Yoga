...
import ReplyComments from "@/Component/admin/modules/ReplyComments";
import BASE_URL from "@/ApiBase/Base";

const Comments = () => {
    const [comments, setComments] = useState([]);
    const [filter, setFilter] = useState("همه");
    const [replyComments, setReplyComments] = useState({show: false, id: null})
    ...
    const handleApprove = async (id) => {
        ...
        try {
            const token = Cookies.get('tokenAdmin');
            const res = await axios.post(`${BASE_URL}/comments/${id}/status`, form, {
                headers: token ? {Authorization: `Bearer ${token}`} : {}
            })
            if (res.status) {
    ...
    const ReplySelectedComment =  (id) => {
        setReplyComments({show: true, id: id})
    }
    ...
    return (
        <div className="p-6 min-h-screen  from-purple-50 to-blue-100">
            ...
            {filteredComments.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {filteredComments.map((comment) => (
                        <div key={comment.id}
                             className={`bg-white/60  backdrop-blur-md border border-white/30 shadow-xl rounded-3xl p-6 flex flex-col justify-between transition-all hover:scale-[1.01]`}>
                    ...
                    {comment.user_name || "کاربر"}
                            ...
                            <div className="flex justify-between gap-3 mt-auto">
                                <button onClick={() => handleApprove(comment.id)}
                                        className="flex-1 bg-green-100 hover:bg-green-200 text-green-800 py-2 rounded-xl text-sm font-medium flex items-center justify-center gap-1 transition">
                                    <FaCheckCircle/>تایید
                                </button>
                                <button
                                    onClick={() => ReplySelectedComment(comment.id)}
                                ...
                                <button onClick={() => handleReject(comment.id)}
                                        className="flex-1 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 py-2 rounded-xl text-sm font-medium flex items-center justify-center gap-1 transition">
                                    <FaTimesCircle/>رد
                                </button>
                            </div>
                        </div>
            ...
            {replyComments.show && (
                <ReplyComments replyComments={replyComments} setReplyComments={setReplyComments}/>
            )}
        </div>
    );
};
